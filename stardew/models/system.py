...
user_role: Table = Table(
    "sys_user_role",
    Base.metadata,
    Column("user_id", String(32), ForeignKey('sys_user.id', ondelete="CASCADE"), primary_key=True),
    Column("role_id", String(32), ForeignKey('sys_role.id', ondelete="CASCADE"), primary_key=True)
)
...
class SysUser(Base):
    ...
    mobile = Column(String(11), nullable=False, default="", comment="手机号")
    gender = Column(SmallInteger, nullable=False, default=GenderEnum.unknown.value, comment="性别， 0：男， 1：女")
    avatar = Column(String(150), nullable=False, default="", comment="头像URI")
    password = Column(String(64), nullable=False, comment="密码")
    status = Column(SmallInteger, nullable=False, default=StatusEnum.enable.value, comment="用户状态，0正常 1禁用或停用")
    is_super = Column(Boolean, nullable=False, default=False, comment="是否为超级管理员")
    remark = Column(String(50), nullable=False, default="", comment="备注")
    del_flag = Column(Boolean, nullable=False, default=False, comment="是否删除,0：未删除， 1：已删除")

    roles = relationship('SysRole', secondary=user_role, backref=backref("users", lazy=True), lazy="select")
...
class SysRole(Base):
    ...
    key = Column(String(50), unique=True, nullable=False, comment="角色编码")
    status = Column(SmallInteger, nullable=False, default=StatusEnum.enable.value, comment="角色状态，0正常，1停用")
    del_flag = Column(Boolean, nullable=False, default=False, comment="删除标志（0:存在 1:删除）")

    # 定义多对多关系
    perms = relationship('SysMenu', secondary=role_menu, backref=backref("roles", lazy=True), lazy="select")
...
class SysMenu(Base):
    ...
    status = Column(SmallInteger, nullable=False, default=StatusEnum.enable.value, comment="菜单状态（0正常 1停用）")
    perm = Column(String(100), default="", nullable=False, comment="对应的权限名")
    icon = Column(String(256), default="", nullable=False, comment="图标")
    del_flag = Column(Boolean, nullable=False, default=False, comment="删除标志（0:存在 1:删除）")
    # 自引用
    parent = relationship('SysMenu', backref='children', remote_side=[id], lazy="select")
